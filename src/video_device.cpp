...
VideoDevice::VideoDevice(const VideoDevice &other) : _fd(-1) {
  ...
  if (_fd == -1) {
    std::stringstream ss;
    ss << "dup('" << other._fd << "') failed - errno=" << errno << " ('"
       << strerror(errno) << "')";
    ROS_ERROR_STREAM(ss.str());
    throw std::runtime_error(ss.str());
  }
...
int VideoDevice::stream_on() {
  ...
  if (rc == -1) {
    ROS_ERROR("ioctl(%d, VIDIOC_STREAMON) failed - errno=%d, %s", _fd, errno,
              strerror(errno));
  }
  return rc;
}
...
int VideoDevice::stream_off() {
  ...
  if (rc == -1) {
    ROS_ERROR("ioctl(%d, VIDIOC_STREAMOFF) failed - errno=%d, %s", _fd, errno,
              strerror(errno));
  }
  return rc;
}
...
int VideoDevice::capabilities(v4l2_capability &capability) {
  ...
  if (rc == -1) {
    ROS_ERROR("ioctl(%d, VIDIOC_QUERYCAP) failed - errno=%d, %s", _fd, errno,
              strerror(errno));
  }
  return rc;
}
...
int VideoDevice::get_format(v4l2_format &format) {
  ...
  if (rc == -1) {
    ROS_ERROR("ioctl(%d, VIDIOC_G_FMT) failed - errno=%d, %s", _fd, errno,
              strerror(errno));
...
int VideoDevice::set_format(const v4l2_format &format) {
  ...
  if (rc == -1) {
    ROS_ERROR("ioctl(%d, VIDIOC_S_FMT) failed - errno=%d, %s", _fd, errno,
              strerror(errno));
  } else {
    _log_format("video device format", format);
  }
